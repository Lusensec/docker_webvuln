...
function luozi($conn,$home_id,$qipan_id,$yidong,$row,$hang,$lie){
//先查询第一次起子的位置
...
if($row){       //房间存在
    ...
    if ($row['yidong']){   //已经起子
        ...
        $row = qipan_data_select($conn, $qipan_id, $hang);  //查询qipan落子位置是否有子
        if ($row[$lie] != 0) {      //落子 位置有子
            $lin = home_data_select($conn,$home_id);
            $xian = $lin['yidong'];
            if ( $xian == $yidong ) {    //起子和落子在一个位置
                failed($conn,$home_id,true);
            } else {
                if ( kong_bai( $conn,$home_id,$xian ,$yidong ) ){  //判断落子是否为羊，且是否处于5个圈中
                    //可落子（只能羊落子）
                    luozi($conn,$home_id,$qipan_id,$yidong,$row,$hang,$lie);
                } else {    //不能落子
                    failed($conn,$home_id,true);
                }
            }
