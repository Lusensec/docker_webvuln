    ...
    if (file_exists($fl_path)) {
        ...
        if (!empty($channel_cate)) {
            foreach ($channel_cate as $key => $value) {
                $arr[] = $value['id'];
                if (in_array($value['cate_parent'], $arr)) {
                    continue;
                }
...
{
    if (empty($file_name) && empty($thumb_width) && $thumb_height) {
        return;
    }
...
{
    if (file_exists(DATA_PATH . $lang . '_tpl_info.php')) {
        include(DATA_PATH . $lang . '_tpl_info.php');
    }
    //$num=count($tpl);
...
function get_verify($verify)
{
    ...
    foreach ($member_group as $k => $v) {
        if ($verify == $v['id'] && !$v['is_disable']) {
            return $v['member_group_name'];
            break;
        }
    }
    return "开放浏览";
