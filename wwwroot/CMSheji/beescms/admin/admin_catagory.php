...
elseif ($action == 'add') {
    ...
    if (!empty($temp_id)) {
        ...
        if ($is_temp_id) {
            msg('<span style="color:red">【已经存在模板标示ID，请更改！】</span>');
        }
    }
...
elseif ($action == 'save_move') {
    ...
    if ($is_child[0]['num']) {
        msg('<span style="color:red">操作失败！请先删除或移动该栏目的下级栏目！</span>');
    }
