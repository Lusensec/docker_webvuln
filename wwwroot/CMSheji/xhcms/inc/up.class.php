...
//user define ------------------------------------- 
    ...
    {// string $prefix , int $mode
        ...
        switch ($mode) {
            case 0   :
                $fullName .= rand(0, 100) . "_" . strtolower(date("ldSfFYhisa"));
                break;
            case 1   :
                $fullName .= rand(0, 100) . "_" . time();
                break;
            case 2   :
                $fullName .= rand(0, 10000) . time();
                break;
            default  :
                $fullName .= rand(0, 10000) . time();
                break;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
        $pass = false;
        $defineTypeList = strtolower($this->defineTypeList);    // 拿到白名单
        $MIME = strtolower($this->GetFileMIME());   // 拿到后缀名，仅仅是文件名最后3个字符
